...
Wake-word tunables (`WAKE_WORD`, `WAKE_WORD_THRESHOLD`, `FRAME_MS`) sit at the top of [reachy_chat/main.py](reachy_chat/main.py); realtime tunables (`REALTIME_MODEL`, `REALTIME_VOICE`, `MAX_TURN_S` per-response cap, `MAX_SESSION_S` whole-session cap, `FOLLOWUP_WINDOW_S` post-response silence window, `LISTENING_WAVE_*` antenna cue params, `RESET_TO_NEUTRAL_DURATION_S`) at the top of [reachy_chat/realtime.py](reachy_chat/realtime.py). `_speak()` and `GREETING` in main.py are kept around for offline diagnostics — not on the happy path.
